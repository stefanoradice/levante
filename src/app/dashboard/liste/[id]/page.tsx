...
export default async function ListDetailPage({ params }: Props) {
  ...
  const lists = await getUserDataRepository()
    .getLists(session.token)
    .catch(() => null);
  const list = lists?.find((l) => l.id === id);
  if (!list) notFound();

  const posts = await getPostRepository()
    .getPostsByIds(list.postIds)
    .catch(() => []);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
      ...
      <p className="text-sm text-foreground-muted mb-10">
        {list.postIds.length}{" "}
        {list.postIds.length === 1 ? "articolo" : "articoli"}
      </p>
